...
from PySide6.QtWidgets import (
    ...
    QComboBox, QSplitter, QSlider, QLineEdit # Adicionado QSlider, QLineEdit
)
from PySide6.QtCore import Qt, QSettings, QTimer
from PySide6.QtGui import QFont, QClipboard, QKeySequence, QShortcut, QColor, QIntValidator # Adicionado QIntValidator
...
class VersionTimeTracker(QMainWindow):
    def __init__(self):
        ...
        # Timer para atualização automática (MOVIDO PARA CIMA)
        # Inicialize o timer antes de criar a seção de input
    ...
    def create_header_section(self):
        ...
        title_label.setFont(QFont("Arial", 28, QFont.Bold)) # Título maior
    ...
    def create_input_section(self):
        ...
        # Botão de processamento (MOVIDO PARA CIMA)
        ...
        break_controls_layout = QHBoxLayout()
        break_controls_layout.addWidget(self.max_break_slider)
        break_controls_layout.addWidget(self.max_break_input)

        settings_layout.addWidget(break_label)
        settings_layout.addLayout(break_controls_layout)
        settings_layout.addStretch()
        settings_layout.addWidget(self.process_btn) # Agora self.process_btn já foi inicializado
    ...
    def process_data(self):
        ...
        try:
            ...
            # print(f"Parsed versions (unsorted): {[v['name'] + ' ' + v['time'].strftime('%H:%M') for v in self.all_versions]}") # DEBUG
            ...
            self.work_blocks, self.total_time = self.calculate_work_blocks() # Modificado para retornar também o total_time
    ...
    def calculate_work_blocks(self):
        ...
        # print("\n--- Debugging calculate_work_blocks ---") # DEBUG
        # print(f"Max Break Minutes: {self.max_break_minutes}") # DEBUG
        ...
        for i in range(1, len(sorted_versions)):
            ...
            # print(f"Prev: {prev_version['name']} ({prev_version['time']}) | Curr: {curr_version['name']} ({curr_version['time']}) | Diff: {time_diff} ({time_diff.total_seconds()/60:.2f} min)") # DEBUG
            ...
            if time_diff.total_seconds() <= self.max_break_minutes * 60:
                ...
                # print(f"  -> Added to total. Current total_active_time: {total_active_time}") # DEBUG
            else:
                # print(f"  -> IGNORING (Break > {self.max_break_minutes} min). Starting new block.") # DEBUG
                ...
                if len(current_block_versions) > 0: # Garante que há versões para formar um bloco
                    block_start_dt = datetime.combine(datetime.today(), current_block_versions[0]['time'])
                    block_end_dt = datetime.combine(datetime.today(), current_block_versions[-1]['time'])
                    if block_end_dt < block_start_dt: # Ajuste para blocos que atravessam meia-noite
                    ...
                    work_blocks.append({
                        ...
                        'duration': block_end_dt - block_start_dt # Duração do bloco inteiro
                    })
        ...
        # print(f"Final calculated total_active_time (before return): {total_active_time}") # DEBUG
        # print("--- End Debugging ---") # DEBUG
        
        return work_blocks, total_active_time
    
    def calculate_block_duration(self, block):
        """Calcula a duração total de um bloco de trabalho (do primeiro ao último item no bloco)"""
        # Esta função ainda é útil para a duração 'real' de um bloco na visualização,
        # mas a duração 'ativa' é calculada no calculate_work_blocks
        if not block:
            return timedelta()
        ...
        # Ajuste para virada de meia-noite dentro de um bloco
    ...
    def display_compact_view(self):
        """Exibe apenas os blocos de trabalho (visão consolidada)"""
        self.result_table.setRowCount(len(self.work_blocks))
        
        for row, block in enumerate(self.work_blocks):
            # Versão inicial
    ...
    def display_expanded_view(self):
        """Exibe todas as versões com marcação de blocos"""
        # Primeiro calcula o número total de linhas (todas as versões)
        total_rows = 0
        for block in self.work_blocks:
            total_rows += len(block['versions'])
        self.result_table.setRowCount(total_rows)
        
        row_index = 0
        for block_idx, block in enumerate(self.work_blocks):
            for version_idx, version in enumerate(block['versions']):
                # Determina se é primeira ou última versão do bloco
                is_first = version_idx == 0
                is_last = version_idx == len(block['versions']) - 1
                
                # Colunas
                version_name = version['name']
                version_time = version['time'].strftime("%H:%M")
                
                # Para versões intermediárias, mostramos apenas o nome e hora
                start_version = version_name if is_first else ""
                start_time = version_time if is_first else ""
                end_version = version_name if is_last else ""
                end_time = version_time if is_last else ""
                
                # Duração do bloco apenas na última linha do bloco
                duration = self.format_duration(block['duration']) if is_last else ""
                ...
                # Destaque visual para blocos
                for item in items:
                    item.setTextAlignment(Qt.AlignCenter)
                    # Aplicar cor de fundo diferente para as linhas de início e fim de bloco
                    # e alternar para as outras linhas
                    if is_first or is_last:
                        item.setBackground(QColor(COLORS['table_header']))
                    elif row_index % 2 == 0:
