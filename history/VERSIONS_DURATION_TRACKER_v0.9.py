...
STRINGS = {
    # 🧠 App identity
    "app_title": "▶ VERSION TIME TRACKER V9",
...
class VersionTimeTracker(QMainWindow):
    def __init__(self):
        ...
        self.setMinimumSize(600, 450) # Further reduced minimum size for more flexibility
        ...
        # Configurações
        self.settings = QSettings("DeepSeek", "VersionTimeTracker") # Corrected QSettings organization name
        ...
        # Aplicar estilo inicial (com fator de escala 1.0)
    ...
    def _generate_stylesheet(self, scaling_factor):
        ...
        # Helper to scale font sizes for QFont objects (used for specific labels)
        def get_scaled_font(base_size):
            return QFont(COLORS["font_family"], self._scale_font_size(base_size, scaling_factor))
        ...
        # Reconstruct the stylesheet string using scaled values
        # Note: For QLabel and QPushButton, if their font is set via QFont directly,
        # the stylesheet's font-size might be overridden.
        # To ensure stylesheet controls all, we remove direct QFont.setFont calls
        # and rely on these global rules.
        ...
        return f"""
            ...
            QHeaderView::section {{
                ...
                border: none;
                font-weight: bold;
                font-size: {self._scale_font_size(COLORS['font_size_medium'], scaling_factor)}pt; /* Ensure header scales */
            }}
            QTableWidget::item {{
                padding: {get_scaled_px(COLORS['padding_sm'])};
            }}
            QPushButton {{
                ...
                padding: {get_scaled_px(COLORS['padding_md'])} {get_scaled_px(COLORS['padding_lg'])};
                font-weight: bold;
                min-width: {get_scaled_px('100px')}; /* Scale min-width too */
                font-size: {self._scale_font_size(COLORS['font_size_medium'], scaling_factor)}pt;
            }}
            QPushButton:hover {{
            ...
            QPushButton:pressed {{
                ...
                /* Recalculate padding for pressed state based on scaled padding_md */
            ...
            QSlider::groove:horizontal {{
                border: {get_scaled_px(COLORS['border_width_thin'])} solid {COLORS['accent']};
                height: {get_scaled_px('8px')}; /* Scale slider groove height */
            ...
            QSlider::handle:horizontal {{
                ...
                width: {get_scaled_px('18px')}; /* Scale slider handle width */
                margin: {get_scaled_px('-5px')} 0; /* Scale margin */
            ...
            /* Specific styles for elements that need different base font sizes */
            #title_label {{
    ...
    def resizeEvent(self, event):
        ...
        # Calculate scaling factor based on current width relative to base width
        scaling_factor = current_width / self.base_width
        self.setStyleSheet(self._generate_stylesheet(scaling_factor))

        # Update main layout spacing based on scaled margin_md
        # Ensure findChild returns a valid object before calling .layout()
        if self.central_widget.layout():
            self.central_widget.layout().setSpacing(int(self._scale_px_value(COLORS["margin_md"], scaling_factor).replace('px','')))
        ...
        # (These are not handled by general QLabel/QPushButton rules if they have specific object names)
        # Re-apply total_label style to ensure padding/border scales
        ...
        # Re-apply subtitle_frame style to ensure padding scales
        self.subtitle_frame.setStyleSheet(f"""
            background-color: {COLORS['table_header']};
