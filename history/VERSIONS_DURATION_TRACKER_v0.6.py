...
COLORS = {
    ...
    "font_size_title": "36px", # Increased for more prominence
...
STRINGS = {
    ...
    # Tooltips (Translated to English)
    "tt_input_text": "Paste your Blender version list here. Each version should have 'Version X', filename, time (HH:MM), and author, separated by blank lines.",
    "tt_process_btn": "Processes the pasted version data to calculate active time blocks. Shortcut: Ctrl+P",
    "tt_max_break_slider": "Set the maximum break time (in minutes) between versions to be considered part of the same work block.",
    "tt_max_break_input": "Manually enter the maximum break time (in minutes).",
    "tt_result_table": "Displays the calculated work blocks. Use compact view for a summary or expanded view for details.",
    "tt_export_btn": "Exports the table results to a CSV or TXT file. Shortcut: Ctrl+E",
    "tt_copy_btn": "Copies the total active time to the clipboard. Shortcut: Ctrl+Shift+C",
    "tt_toggle_view_btn": "Toggles between compact view (consolidated blocks) and expanded view (all versions with block markers). Shortcut: Ctrl+T",
    "tt_total_label": "The total active time, excluding breaks longer than the defined limit.",
    "tt_exit_btn": "Exits the program. Shortcut: Ctrl+Q"
}
...
class VersionTimeTracker(QMainWindow):
    def __init__(self):
        ...
        # Inicializa o botão de processo aqui para que seja acessível em create_result_section
        ...
        self.main_layout.addWidget(self.exit_btn, alignment=Qt.AlignRight)
    ...
    def create_header_section(self):
        ...
        title_label.setFont(QFont(COLORS["font_family"], int(COLORS["font_size_title"].replace('px','')), QFont.Bold))
    ...
    def create_input_section(self):
        ...
        # Removido settings_layout daqui, pois Max Break e Process button foram movidos
    ...
    def create_result_section(self):
        ...
        # Seletor de tempo máximo de pausa (Layout ajustado e movido para cá)
        max_break_controls_widget = QWidget()
        max_break_layout = QVBoxLayout(max_break_controls_widget)
        max_break_layout.setContentsMargins(0, 0, 0, 0)
        max_break_layout.setSpacing(int(COLORS["margin_sm"].replace('px','')))

        max_break_label_input_layout = QHBoxLayout()
        ...
        max_break_label_input_layout.addWidget(break_label)
        max_break_label_input_layout.addStretch()
        max_break_label_input_layout.addWidget(self.max_break_input)
        ...
        max_break_layout.addLayout(max_break_label_input_layout)
        max_break_layout.addWidget(self.max_break_slider)
        ...
        result_header_layout.addWidget(max_break_controls_widget) # Adiciona a área de Max Break aqui
        result_header_layout.addStretch() # Empurra o toggle_view_btn para a direita
        ...
        # Botões (Process, Export, Copy)
        button_layout = QHBoxLayout()
        
        # Botão de Processar (movido para cá)
        button_layout.addWidget(self.process_btn) # self.process_btn já foi inicializado em __init__
    ...
    def apply_dark_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton {{
                ...
                border: {COLORS['border_width_thin']} solid {COLORS['accent']}; /* Added border */
                border-radius: {COLORS['border_radius_sm']};
                padding: {COLORS['padding_md']} {COLORS['padding_lg']};
                font-weight: bold;
                min-width: 100px;
                box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.3); /* Added subtle shadow */
            }}
            QPushButton:hover {{
                background-color: {COLORS['accent']};
                box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.5); /* Stronger shadow on hover */
            }}
            QLabel {{
                font-size: {COLORS['font_size_medium']};
            }}
    ...
    def display_expanded_view(self):
        ...
        for i in range(len(sorted_versions_desc) - 1):
            ...
            if time_diff.total_seconds() / 60 > self.max_break_minutes:
                duration_str += f" (IGNORED - Break > {self.max_break_minutes} min)" # Translated to English
