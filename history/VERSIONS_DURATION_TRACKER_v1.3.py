...
class VersionProcessorWorker(QObject):
    ...
    def _parse_versions(self, text):
            ...
            for block_idx, block in enumerate(blocks):
                ...
                for i, line in enumerate(lines):
                    ...
                    if (line.lower().startswith("version") or line.lower().startswith("current")):
                        ...
                        if line.lower() == "current version" and (i + 1) < len(lines):
                            ...
                            print(f"DEBUG: _parse_versions - Found 'Current version', name: {version_name}")
                        else:
                            version_name = line # Otherwise, the version name is on this line
                            print(f"DEBUG: _parse_versions - Found version line, name: {version_name}")
                    ...
                    if len(line) == 5 and line[2] == ':':
                        try:
                            ...
                            time_str = line
                            print(f"DEBUG: _parse_versions - Found HH:MM time: {time_str}")
                        except ValueError:
                            pass # Not a valid HH:MM, continue searching
                    elif len(line) == 4 and line[1] == ':': # Handles H:MM format (e.g., 9:30)
                        try:
                            datetime.strptime(f"0{line}", "%H:%M") # Prepend '0' for parsing
                            time_str = f"0{line}"
                            print(f"DEBUG: _parse_versions - Found H:MM time: {time_str}")
                        except ValueError:
                            pass # Not a valid H:MM, continue searching
                ...
                if version_name and time_str:
                    try:
                        ...
                        print(f"DEBUG: _parse_versions - Added version: {version_name}, time: {time_obj}")
                    except ValueError:
            ...
            if not versions:
                print("DEBUG: _parse_versions - No valid versions found after parsing all blocks.")
                raise ValueError("No valid versions found")
            
            print(f"DEBUG: _parse_versions - Successfully parsed {len(versions)} versions.")
            return versions
    
    def _parse_time_ranges_string(self, text):
        """
        Parses a string like "11h até as 11h46 + 11h50 às 12h26 + ..."
        ...
        # Regex to match "HHhMM às HHhMM" or "HHh até as HHhMM"
        # Group 1: start hour, Group 2: start minute (optional)
        # Group 3: end hour, Group 4: end minute
        time_pattern = re.compile(r'(\d{1,2})h(?:(\d{2}))?\s*(?:até as|às)\s*(\d{1,2})h(\d{2})')
    ...
    def _calculate_work_blocks(self, all_versions, max_break_minutes):
        ...
        print(f"DEBUG: _calculate_work_blocks - Sorted versions: {[v['time'].strftime('%H:%M') for v in sorted_versions]}")
        ...
        print(f"DEBUG: _calculate_work_blocks - Starting first block with: {sorted_versions[0]['name']} at {sorted_versions[0]['time'].strftime('%H:%M')}")
        ...
        for i in range(1, len(sorted_versions)):
            ...
            if curr_time_dt < prev_time_dt:
                ...
                print(f"DEBUG: _calculate_work_blocks - Midnight rollover detected between {prev_version['time'].strftime('%H:%M')} and {curr_version['time'].strftime('%H:%M')}. Adjusted curr_time_dt: {curr_time_dt}")

            time_diff = curr_time_dt - prev_time_dt
            print(f"DEBUG: _calculate_work_blocks - Time diff between {prev_version['name']} ({prev_version['time'].strftime('%H:%M')}) and {curr_version['name']} ({curr_version['time'].strftime('%H:%M')}): {time_diff} ({time_diff.total_seconds() / 60:.2f} min)")
            ...
            if time_diff.total_seconds() / 60 <= max_break_minutes:
                ...
                print(f"DEBUG: _calculate_work_blocks - Added {curr_version['name']} to current block. Current block size: {len(current_block_versions)}")
            else:
                ...
                print(f"DEBUG: _calculate_work_blocks - Started new block with: {curr_version['name']} at {curr_version['time'].strftime('%H:%M')}")
    ...
    def _calculate_block_duration(self, block, max_break_minutes):
        ...
        print(f"DEBUG: _calculate_block_duration - Calculating duration for block starting {sorted_block[0]['name']} at {sorted_block[0]['time'].strftime('%H:%M')}, ending {sorted_block[-1]['name']} at {sorted_block[-1]['time'].strftime('%H:%M')}")
        ...
        for i in range(1, len(sorted_block)):
            ...
            if curr_time < prev_time:
                ...
                print(f"DEBUG: _calculate_block_duration - Midnight rollover in block between {sorted_block[i-1]['time'].strftime('%H:%M')} and {sorted_block[i]['time'].strftime('%H:%M')}. Adjusted curr_time: {curr_time}")
...
class VersionTimeTracker(QMainWindow):
    ...
    def _generate_stylesheet(self, scaling_factor):
        ...
        return f"""
            ...
            QSlider::handle:horizontal {{
                ...
                width: {self._scale_px_value('18px', scaling_factor)};
                margin: {self._scale_px_value('-5px', scaling_factor)} 0;
                border-radius: {self._scale_px_value(COLORS['border_radius_lg'], scaling_factor)};
            }}
    ...
    def create_input_section(self):
        ...
        self.input_text.setPlaceholderText("Example:\nVersion 33\nfilename.blend\n18:15\nAuthor\n\nVersion 32\nfilename.blend\n18:10\nAuthor\n\nOR:\n11h até as 11h46 + 11h50 às 12h26") # Updated placeholder
    ...
    def _handle_processing_results(self, result_dict):
        ...
        if result_type == 'versions':
            ...
            self.subtitle_label.setText("CALCULATING TIME RANGES") # Custom subtitle for this mode
    ...
    def _reset_ui_state_immediate(self):
        ...
        print("DEBUG: _reset_ui_state_immediate - UI state immediate reset complete.")
    ...
    def _update_selected_duration_sum(self):
        ...
        for i in range(self.result_table.columnCount()):
            ...
            # Check for both "DURATION" (for versions) and "DURATION" (for time ranges)
            if header_text == STRINGS["col_duration"] or header_text == "DURATION": # "DURATION" is the hardcoded one in time_ranges view
